...
- **CMS**: Blog posts and images are served from a Strapi-compatible backend. The site fetches content via `utils/api` (`getPosts`, `getPost`) using `NEXT_PUBLIC_CMS_URL` and `NEXT_PUBLIC_CMS_API_KEY`. Image domains for the CMS are configured in `next.config.ts`.
...
cp env.example .env
# Edit .env: NEXT_PUBLIC_CMS_URL, NEXT_PUBLIC_CMS_API_KEY
...
The app is deployable to [Vercel](https://vercel.com). Set `NEXT_PUBLIC_CMS_URL` and `NEXT_PUBLIC_CMS_API_KEY` in the project environment.
